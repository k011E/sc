...
<table width="100%" border="3">
	...
	while($order = $query->fetch_assoc()){
		...
		<tr bgcolor="<?=$color?>">
			...
			<td><?if($order['status']==0){?><a href="/admin/orders/close/<?=$order['id']?>">Закрыть</a> /<?}?> <a href="/admin/orders/delete/<?=$order['id']?>">Удалить</a> / <a href="/admin/orders/edit/<?=$order['id']?>">Редактировать</a></td>
		</tr>
		<?
	}
	?>
